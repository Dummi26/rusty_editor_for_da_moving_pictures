...
impl EditorWindowLayoutContentTrait for QVidRunner {
    ...
    fn draw_onto_custom(&mut self, draw_opts: &mut crate::gui::speedy2d::layout::EditorWindowLayoutContentDrawOptions, graphics: &mut speedy2d::Graphics2D, position: &(f32, f32, f32, f32), input: &mut crate::gui::speedy2d::layout::UserInput) {
        ...
        for command in self.command_handler.refresh() {
            match command {
                crate::gui::quick_commands::QctCommand::UnsetEditing => self.data().requests.push(crate::gui::speedy2d::request::EditorWindowLayoutRequest::DeselectForEditing),
                crate::gui::quick_commands::QctCommand::SetEditingTo(i) => self.data().requests.push(crate::gui::speedy2d::request::EditorWindowLayoutRequest::SelectForEditing(i)),
                crate::gui::quick_commands::QctCommand::SetQueryTo(new_query) => self.query = new_query,
        ...
        for (index, text) in self.command_handler.completions.iter().enumerate() {
            ...
            graphics.draw_text(Vector2::new(position.0, position.1 + position.3 + index as f32 * line_height),
                ...
                crate::gui::speedy2d::layout::KeyboardAction::Typed(ch) => {
                    if !(input.owned.keyboard_modifiers_state.logo() || input.owned.keyboard_modifiers_state.ctrl() || input.owned.keyboard_modifiers_state.alt()) {
                        ...
                        let changed = match useful::CharOrAction::from(ch) {
                            useful::CharOrAction::Char(ch) => { self.query.push(ch.clone()); true }
                            ...
                            useful::CharOrAction::Backspace => { self.query = { let mut vec = Vec::from_iter(self.query.chars()); vec.pop(); String::from_iter(vec.into_iter()) }; true },
                                ...
                                if !self.command_handler.completions.is_empty() {
                                    ...
                                    if let Some((_, completions_index)) = &mut self.real_query {
                                        if input.owned.keyboard_modifiers_state.shift() == false {
                                            *completions_index += 1;
                                            if *completions_index >= self.command_handler.completions.len() {
                                        ...
                                        if !revert_real_query {
                                            self.query = self.command_handler.completions[*completions_index].clone();
                                        }
                                    } else {
                                        let i = if input.owned.keyboard_modifiers_state.shift() {
                                            self.command_handler.completions.len() - 1
                                        } else { 0 };
                                        self.real_query = Some((std::mem::replace(&mut self.query, self.command_handler.completions[i].clone()), i));
                                    }
                                    if revert_real_query {
                        ...
                        if exec { // exec BEFORE setting new query!
                            self.command_handler.exec_query(if let Some((_, index)) = self.real_query { Some(index) } else { None });
                        }
                        if changed {
...
impl QVidRunner {
    pub fn new(mode: QVidRunnerMode, selected: Option<u32>, video: std::sync::Arc<std::sync::Mutex<crate::video::Video>>) -> Self {
        Self {
            mode,
            query: String::new(),
            real_query: None,
            command_handler: crate::gui::quick_commands::QuickCommandsHandler::new(selected, video),
