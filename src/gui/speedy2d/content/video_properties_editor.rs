...
use speedy2d::{
    color::Color,
    dimen::Vector2,
    font::{TextAlignment, TextLayout, TextOptions},
};

use crate::{
    content::{content::Content, image::ImageChanges},
    curve::Curve,
    effect::{self, effects::EffectT},
    gui::speedy2d::{
        content_list::{EditorWindowLayoutContent, EditorWindowLayoutContentEnum},
        layout::{
            CustomDrawActions, EditorWindowLayoutContentData, EditorWindowLayoutContentDrawMode,
            EditorWindowLayoutContentSDrawMode, EditorWindowLayoutContentTrait, MouseAction,
        },
        request::EditorWindowLayoutRequest,
    },
    useful,
    video::{
        Video, VideoChanges, VideoType, VideoTypeChanges, VideoTypeChanges_List, VideoTypeEnum,
    },
};
...
impl ExtraTabsInfo for EtGeneral {
    ...
    fn draw_icon(
        ...
        shared_data: &SharedEtData,
    ) {
        graphics.draw_line(
            Vector2 {
                x: position.0 + (hovered + selected).min(1.0) * position.2,
                y: position.1,
            },
            Vector2 {
                x: position.0 + position.2 - selected * position.2,
                y: position.1 + position.3,
            },
            1.0,
            shared_data.unified_color,
        );
    }
    fn draw(
        &mut self,
        supr: &mut VideoPropertiesEditor,
        ...
        for (opt, txt) in opts.into_iter().enumerate() {
            ...
            graphics.draw_text(
                ...
                Color::from_rgba(1.0, 1.0, 1.0, vis),
                &draw_opts.assets_manager.get_default_font().layout_text(
                    txt,
                    h * 0.5,
                    TextOptions::new(),
                ),
        ...
        match &input.owned.action {
            ...
            crate::gui::speedy2d::layout::InputAction::Mouse(action) => match action {
                ...
                MouseAction::ButtonUp(btn) => {
                    ...
                    let mouse_index = if 0.0 < mouse_pos.0
                        && mouse_pos.0 < 1.0
                        && 0.0 < mouse_pos.1
                        && mouse_pos.1 < 1.0
                    {
                        Some(
                            ((mouse_pos.1 * possibilities as f32).floor() as usize)
                                .min(possibilities - 1),
                        )
                    } else {
                        None
                    ...
                    if let Some(mouse_index) = mouse_index {
                        match mouse_index {
                            0 => {
                                if let Some(index) =
                                    VideoPropertiesEditor::get_extra_tabs_index_where(
                                        &supr.tabs,
                                        |e| false,
                                    )
                                {
                                    ...
                                    supr.tabs.push(Some(Box::new(EtCurve::new(
                                        "x".to_string(),
                                        0,
                                        |v, c| {
                                            v.set_pos.x = c;
                                        },
                                        supr.editing.0.as_ref().unwrap().1.set_pos.x.clone(),
                                    ))));
                                };
                            }
                            1 => {
                                    ...
                                    )
                                {
                                    ...
                                    supr.tabs.push(Some(Box::new(EtCurve::new(
                                        ...
                                        supr.editing.0.as_ref().unwrap().1.set_pos.y.clone(),
                                    ))));
                                };
                            }
                            2 => {
                                    ...
                                    )
                                {
                                    ...
                                    supr.tabs.push(Some(Box::new(EtCurve::new(
                                        ...
                                        supr.editing.0.as_ref().unwrap().1.set_pos.w.clone(),
                                    ))));
                                };
                            }
                            3 => {
                                    ...
                                    )
                                {
                                    ...
                                    supr.tabs.push(Some(Box::new(EtCurve::new(
                                        ...
                                        supr.editing.0.as_ref().unwrap().1.set_pos.h.clone(),
                                    ))));
                                };
                            }
                            4 => {
                                    ...
                                    )
                                {
                                    ...
                                    supr.tabs.push(Some(Box::new(EtTime::new_len(
                                        editing.1.set_start_frame,
                                        editing.1.set_length,
                                    ))));
                                };
                            }
                            5 => {
...
impl ExtraTabsInfo for EtTime {
    ...
    fn draw(
        ...
        graphics.draw_line(
            Vector2 {
                x: position.0 + 0.05 * position.2,
                y,
            },
            Vector2 {
                x: position.0 + (0.05 + 0.9 * self.start as f32) * position.2,
                y,
            },
            per_item_height * 0.1,
            Color::from_rgba(
                if editing_video.set_start_frame == self.start {
                    0.5
                } else {
                    1.0
                },
        ...
        graphics.draw_line(
            ...
            Vector2 {
                x: position.0 + (0.05 + 0.9 * self.end as f32) * position.2,
                y,
            },
            per_item_height * 0.1,
            Color::from_rgba(
                if editing_video.set_length == self.end - self.start {
                    0.5
                } else {
                    1.0
                },
        ...
        let text = font.layout_text(
            format!("from {}\nto {}", self.start, self.end).as_str(),
            per_item_height * 0.9,
            TextOptions::new().with_wrap_to_width(position.2, TextAlignment::Center),
        );
        graphics.draw_text(
            Vector2 {
                x: position.0,
        ...
        graphics.draw_text(
            Vector2 {
                x: position.0,
                y: position.1 + position.3 - text.height(),
            },
            Color::from_rgba(1.0, 1.0, 1.0, vis),
            &text,
        );
            ...
            crate::gui::speedy2d::layout::InputAction::Mouse(action) => match action {
                MouseAction::Moved | MouseAction::ButtonUp(speedy2d::window::MouseButton::Left) => {
                    if input
                        .owned
                        .mouse_down_buttons
                        .contains_key(&speedy2d::window::MouseButton::Left)
                    {
                        if 0.0 < mouse_pos.0
                            && mouse_pos.0 < 1.0
                            && 0.0 < mouse_pos.1
                            && mouse_pos.1 < 1.0
                        {
                            match (mouse_pos.1 * 4.0).floor() as i32 {
                                ...
                                1 => {
                                    self.start =
                                        ((mouse_pos.0 as f64 - 0.05) / 0.9).max(0.0).min(self.end)
                                }
                                2 => {
                                    self.end =
                                        ((mouse_pos.0 as f64 - 0.05) / 0.9).max(self.start).min(1.0)
                                }
                                3 => {
                                    supr.data().requests.push(
                                        EditorWindowLayoutRequest::EditingChangesApply(
                                            VideoChanges {
                                                pos: None,
                                                start: Some(self.start),
                                                length: Some(self.end - self.start),
                                                video: None,
                                                wrap: None,
                                                replace: None,
                                            },
            ...
            if let Some((_, editing)) = &supr.editing.0 {
                self.possibilities = Some(match &editing.video.vt {
                    VideoTypeEnum::List(..) => vec![
                        (
                            "first element in aspect ratio".to_string(),
                            Some(crate::video::VideoChangesReplaceWith::AspectRatio),
                        ),
                        (
                            "first element with effect".to_string(),
                            Some(crate::video::VideoChangesReplaceWith::WithEffect),
                        ),
                    ],
                    VideoTypeEnum::AspectRatio(..) => vec![(
                        "list".to_string(),
                        Some(crate::video::VideoChangesReplaceWith::List),
                    )],
                    ...
                    VideoTypeEnum::Text(t) => match t.text() {
                        crate::content::text::TextType::Static(_) => vec![
                            (
                                "image (using this as its path)".to_string(),
                                Some(crate::video::VideoChangesReplaceWith::Image),
                            ),
                            (
                                "video (with this as its path)".to_string(),
                                Some(crate::video::VideoChangesReplaceWith::Ffmpeg),
                            ),
                        ],
                        crate::content::text::TextType::Program(_) => vec![
                            (
                                "image (using the programs path)".to_string(),
                                Some(crate::video::VideoChangesReplaceWith::Image),
                            ),
                            (
                                "video (using the programs path)".to_string(),
                                Some(crate::video::VideoChangesReplaceWith::Ffmpeg),
                            ),
                        ],
                    },
                    VideoTypeEnum::Image(..) => vec![
                        (
                            "text (showing the path)".to_string(),
                            Some(crate::video::VideoChangesReplaceWith::Text),
                        ),
                        (
                            "video (with the same path)".to_string(),
                            Some(crate::video::VideoChangesReplaceWith::Ffmpeg),
                        ),
                    ],
                    VideoTypeEnum::Raw(..) => vec![
                        ...
                        (
                            "image (with the same path)".to_string(),
                            Some(crate::video::VideoChangesReplaceWith::Image),
                        ),
                    ],
                        ...
                        if let Some(possibilities) = &self.possibilities {
                            if possibilities.len() > 0 {
                                ...
                                if mx >= 0.0 && mx <= 1.0 && my >= 0.0 && my <= 1.0 {
                                    if let Some((_, Some(p))) = possibilities
                                        .get((my * possibilities.len() as f32) as usize)
                                    {
...
impl ExtraTabsInfo for EtCurve {
    fn title(&self) -> StringTypeForExtraTabTitle {
        format!("edit {}-curve", self.name).into()
    }
    fn draw_icon(
        &mut self,
        vis: f32,
    ...
    fn draw(
        ...
        for i in 0..diagram_width {
            let v = self
                .curve
                .get_value(i as f64 / diagram_width_minus_one_float);
            if v > max {
                max = v;
            }
            if v < min {
        ...
        let mut prev_point = None;
        {
            ...
            graphics.draw_line(
                ...
                1.0,
                Color::from_rgba(0.7, 1.0, 0.7, vis),
            );
            graphics.draw_line(
                Vector2 { x: left, y: y2 },
                Vector2 { x: right, y: y2 },
                1.0,
                Color::from_rgba(0.7, 0.7, 1.0, vis),
            );
        }
        for (i, v) in values.iter().enumerate() {
            let v = if vis < 1.0 {
                // on fadein/fadeout, smoothly fade to a flat line of y=0.5
                0.5 + (v - 0.5) * (vis * vis) as f64
            } else {
                *v
            };
            let mut pos_y = position.1 + position.3 * ((max - v) / minmaxdiff) as f32;
            let this_vec = Vector2 {
                x: position.0 + i as f32,
                y: pos_y,
            };
            if let Some(prev) = prev_point.take() {
                graphics.draw_line(prev, this_vec, 1.0, Color::from_rgba(1.0, 1.0, 1.0, vis));
            }
...
impl ExtraTabsInfo for EtListAdd {
    ...
    fn draw_icon(
        ...
        let (sin, cos) = if hovered == 0.0 && (selected == 0.0 || selected == 1.0) {
            ...
            let ang = std::f32::consts::PI
                * (hovered + hovered * hovered - 1.5 * hovered * hovered * hovered
                    + 0.5 * selected);
            (ang.sin(), ang.cos())
        };
        ...
        let mid = Vector2 {
            x: position.0 + 0.5 * position.2,
            y: position.1 + 0.5 * position.3,
        };
        let p1 = Vector2 {
            x: mid.x + sin * h,
            y: mid.y + cos * h,
    ...
    fn draw(
        ...
        let possibilities = [
            (
                "List",
                "Holds multiple objects, applying its own size and position to all of them.",
            ),
            ("Effect", "Applies effects to an object."),
            ("Image", "Displays a static image"),
            ("Video", "Displays a video."),
        ];
        let mouse_index = if 0.0 < input.clonable.mouse_pos.0
            && input.clonable.mouse_pos.0 < 1.0
            && 0.0 < input.clonable.mouse_pos.1
            && input.clonable.mouse_pos.1 < 1.0
        {
            Some(
                ((input.clonable.mouse_pos.1 * possibilities.len() as f32).floor() as usize)
                    .min(possibilities.len() - 1),
            )
        } else {
            None
        };
        ...
        for (possibility, text) in possibilities.into_iter().enumerate() {
            ...
            graphics.draw_line(
                ...
                1.0,
                Color::from_rgba(0.5, 0.5, 0.5, vis),
            );
            graphics.draw_text(
                Vector2 { x: position.0, y },
                Color::from_rgba(
                    if hover { 1.0 } else { 0.7 },
                    if hover { 1.0 } else { 0.7 },
                    if hover { 1.0 } else { 0.7 },
                    vis,
                ),
            ...
            graphics.draw_text(
                ...
                Color::from_rgba(0.7, 0.7, 0.7, vis),
                &draw_opts.assets_manager.get_default_font().layout_text(
                    text.1,
                    h * 0.25,
                    TextOptions::new().with_wrap_to_width(position.2, TextAlignment::Left),
                ),
            );
        }
    }
            ...
            crate::gui::speedy2d::layout::InputAction::Mouse(action) => match action {
                ...
                MouseAction::ButtonUp(btn) => {
                    ...
                    if let Some(mouse_index) = mouse_index {
                        ...
                        let gcd = supr.video.lock().unwrap().generic_content_data().clone();
                        let inner_changes = match mouse_index {
                            0 => Some(VideoTypeChanges_List::Insert(
                                0,
                                Video::new_full(VideoType::new(
                                    VideoTypeEnum::List(Vec::new()),
                                    gcd,
                                )),
                            )),
                            1 => Some(VideoTypeChanges_List::Insert(
                                0,
                                Video::new_full(VideoType::new(
                                    VideoTypeEnum::WithEffect(
                                        ...
                                        effect::Effect::new(
                                            effect::effects::Nothing::new().as_enum(),
                                        ),
                                    ),
                                    gcd,
                                )),
                            )),
                            2 => Some(VideoTypeChanges_List::Insert(
                                0,
                                Video::new_full(VideoType::new(
                            ...
                            3 => Some(VideoTypeChanges_List::Insert(
                                0,
                                Video::new_full(VideoType::new(
                                    VideoTypeEnum::Raw(
                                        crate::content::input_video::InputVideo::new(gcd.reset()),
                                    ),
                                    gcd,
                                )),
                            )),
                        ...
                        if let Some(inner_changes) = inner_changes {
                            let changes = VideoChanges {
                                ...
                                start: None,
                                length: None,
                                video: Some(VideoTypeChanges::List(vec![inner_changes])),
                                wrap: None,
                                replace: None,
                            };
                            supr.data()
                                .requests
                                .push(EditorWindowLayoutRequest::EditingChangesApply(changes));
                        };
                    };
                }
                MouseAction::Scroll(_) => (),
...
impl ExtraTabsInfo for EtPath {
    ...
    fn draw(
        ...
        let path_text = font.layout_text(
            &self.path.to_string_lossy(),
            per_item_height * 0.7,
            TextOptions::new().with_wrap_to_width(position.2, TextAlignment::Left),
        );
        let color = match (supr.has_keyboard_focus, self.path != self.og_path) {
            (false, false) => Color::from_rgba(0.8, 0.8, 0.8, vis),
            (true, false) => Color::from_rgba(1.0, 1.0, 1.0, vis),
            (true, true) => Color::from_rgba(1.0, 0.8, 0.8, vis),
            (false, true) => Color::from_rgba(1.0, 0.5, 0.5, vis),
        };
        graphics.draw_text(
            Vector2 {
                x: position.0,
                ...
                crate::gui::speedy2d::layout::KeyboardAction::Typed(ch) => {
                    match useful::CharOrAction::from(ch) {
                        ...
                        useful::CharOrAction::Enter => {
                            supr.data().requests.push(
                                EditorWindowLayoutRequest::EditingChangesApply(VideoChanges {
...
impl EditorWindowLayoutContentTrait for VideoPropertiesEditor {
    ...
    fn draw_onto_custom(
        ...
        for action in input.get_custom_actions().unwrap() {
            match action {
                ...
                | val @ CustomDrawActions::ChangedVideo => {
                    let new_index = if let CustomDrawActions::SetEditingTo(new_index) = val {
                        *new_index
                    } else {
                        if let Some(ed) = &self.editing.0 {
                            Some(ed.0)
                        } else {
                            None
                        }
                    ...
                    self.editing = (
                        if let Some(index) = new_index {
                            ...
                            let new_vid = useful::get_elem_from_index_recursive_mut(
                                &mut self.video.lock().unwrap(),
                                &mut index.clone(),
                            )
                            .unwrap()
                            .clone_no_caching();
                            if is_new_vid {
                                self.tabs = match &new_vid.video.vt {
                                    VideoTypeEnum::List(_) => vec![
                                        Some(Box::new(EtGeneral::new())),
                                        Some(Box::new(EtPlaceholder::new())), /*ExtraTabsInfo::ListEdit*/
                                        Some(Box::new(EtListAdd::new())), /*ExtraTabsInfo::ListAdd*/
                                    ],
                                    VideoTypeEnum::AspectRatio(..) => vec![
                                        Some(Box::new(EtGeneral::new())),
                                        Some(Box::new(EtPlaceholder::new())), /*width*/
                                        Some(Box::new(EtPlaceholder::new())), /*height*/
                                    ],
                                    VideoTypeEnum::WithEffect(_, _) => {
                                        vec![Some(Box::new(EtGeneral::new()))]
                                    }
                                    VideoTypeEnum::Text(_) => vec![
                                        Some(Box::new(EtGeneral::new())),
                                        Some(Box::new(EtPlaceholder::new())),
                                    ],
                                    VideoTypeEnum::Image(img) => vec![
                                        Some(Box::new(EtGeneral::new())),
                                        Some(Box::new(EtPath::new_path(img.path().clone()))), /*ExtraTabsInfo::ImagePath(img.path().clone(), false)*/
                                    ],
                                    VideoTypeEnum::Raw(_) => vec![
                                        Some(Box::new(EtGeneral::new())),
                                        Some(Box::new(EtPlaceholder::new())),
                                    ],
                                    VideoTypeEnum::Ffmpeg(_) => vec![
                                        Some(Box::new(EtGeneral::new())),
                                        Some(Box::new(EtPlaceholder::new())),
                                    ],
                                };
        ...
        fn draw_type_preview(
            vis: f32,
            moving: f32,
            graphics: &mut speedy2d::Graphics2D,
            position: &(f32, f32, f32, f32),
        ) {
            fn pt(x: f32, y: f32, d: &(f32, f32, f32, f32)) -> Vector2<f32> {
                Vector2::new(d.0 + x * d.2, d.1 + y * d.3)
            }
    ...
    fn handle_input_custom(
        ...
        match &input.owned.action {
            ...
            crate::gui::speedy2d::layout::InputAction::Mouse(action) => match action {
                crate::gui::speedy2d::layout::MouseAction::Moved => {
                    if 0.0 < input.clonable.mouse_pos.0
                        && input.clonable.mouse_pos.0 < 1.0
                        && 0.0 < input.clonable.mouse_pos.1
                        && input.clonable.mouse_pos.1 < 1.0
                    {
                ...
                crate::gui::speedy2d::layout::MouseAction::Scroll(dist) => {
                    if let EditorWindowLayoutContentDrawMode::Static(
                        EditorWindowLayoutContentSDrawMode::Normal,
                    ) = draw_opts.draw_mode
                    {
                        if 0.0 < input.clonable.mouse_pos.0
                            ...
                            && input.clonable.mouse_pos.1 < 1.0
                        {
                            match dist {
                                ...
                                speedy2d::window::MouseScrollDistance::Pixels { x: _, y, z: _ } => {
                                    ...
                                    self.scroll_dist += *y as f32
                                        / self.get_height_of_element_abs(
                                            draw_opts.my_size_in_pixels.1,
                                        );
                                }
                                speedy2d::window::MouseScrollDistance::Pages { x: _, y, z: _ } => {
                                    // 1 page = amount of elements visible
                                    self.scroll_dist += *y as f32
                                        / self.get_height_of_element_rel(
                                            draw_opts.my_size_in_pixels.1,
                                        );
                                }
                            };
        ...
        if let Some(tab) = self.tabs.get_mut(index) {
            if let Some(mut tab) = tab.take() {
                let mouse_pos = self.get_inner_mouse_position(
                    draw_opts.my_size_in_pixels.1,
                    &input.clonable.mouse_pos,
                );
...
impl VideoPropertiesEditor {
    fn draw_normal(
        ...
        if top_bar_height_rel + tab_bar_height_rel < 1.0 {
            ...
            input.clonable.mouse_pos.1 = self
                .get_inner_mouse_position(draw_opts.my_size_in_pixels.1, &input.clonable.mouse_pos)
                .1;
        ...
        if !self.tabs.is_empty() {
            ...
            if self.tabs_info.len() != num_of_tabs {
                // ensure self.tabs_info.len() == num_of_tabs
                while self.tabs_info.len() < num_of_tabs {
                    self.tabs_info.push(AnyTabInfo::default());
                }
                while self.tabs_info.len() > num_of_tabs {
                    self.tabs_info.pop();
                }
            ...
            for line in 0..lines {
                ...
                graphics.draw_line(
                    ...
                    Vector2::new(x + w, y),
                    1.0,
                    Color::from_rgba(0.8, 0.8, 0.8, vis),
                );
                ...
                graphics.draw_line(
                    ...
                    1.0,
                    Color::from_rgba(0.6, 0.6, 0.6, vis),
                );
                for tab_in_line in 0..tabs_this_line {
                    ...
                    let hovered = match (tab_info.hovered, tab_info.hovered_changed) {
                        ...
                        (false, None) => 0.0,
                        (true, Some(t)) => {
                            let prog = t.elapsed().as_secs_f32()
                                / self.tab_hover_change_duration.as_secs_f32();
                            if prog >= 1.0 {
                    ...
                    let (selected, reset_tab_old) = match &self.tab {
                        ...
                        (tab, Some((old_tab, time))) => {
                            if (index == *tab) != (index == *old_tab) {
                                let progress = (time.elapsed().as_secs_f32()
                                    / self.tab_change_duration.as_secs_f32())
                                .min(1.0);
                                (
                                    if index == *tab {
                                        progress
                    ...
                    if hovered > 0.0 {
                        let text =
                            font.layout_text(tab.title().as_ref(), 0.45 * h, TextOptions::new());
                        graphics.draw_text(
                            Vector2 {
                                x: position.0 + (position.2 - text.width()) / 2.0,
                                y: y_tab_bar_top - text.height(),
                            },
                            Color::from_rgba(
                                tab_color.r(),
                                tab_color.g(),
                    ...
                    tab.draw_icon(
                        ...
                        selected,
                        graphics,
                        &(x, y, line_height, line_height),
                        &SharedEtData {
                            unified_color: tab_color,
                        },
                    );
                    x += line_height;
                    graphics.draw_line(
        ...
        /* draw the header text */
        {
            let txt = font.layout_text(
                match &self.editing.0 {
                    Some((_, vid)) => {
                        let mut s = "Editing: ".to_string();
                        s.push_str(match &vid.video.vt {
            ...
            graphics.draw_text(
                Vector2::new(position.0 + x_offset, position.1),
                Color::from_rgba(1.0, 1.0, 1.0, vis),
                &txt,
            );
        };
    ...
    pub fn get_extra_tabs_index_where(
        tabs: &Vec<Option<Box<dyn ExtraTabsInfo>>>,
        f: fn(&dyn ExtraTabsInfo) -> bool,
    ) -> Option<usize> {
    ...
    pub fn get_inner_position(&self, position: &(f32, f32, f32, f32)) -> (f32, f32, f32, f32) {
        ...
        (
            position.0,
            position.1 + top_bar_height,
            position.2,
            position.3 - top_bar_height - tab_bar_height,
        )
    }
    ...
    pub fn get_inner_mouse_position(&self, height: f32, mouse_pos: &(f32, f32)) -> (f32, f32) {
        ...
        (
            mouse_pos.0,
            (mouse_pos.1 - top_bar_height_rel) / (1.0 - top_bar_height_rel - tab_bar_height_rel),
        )
    }
    ...
    pub fn from_mouse_pos_adjust_highlighting_and_get_index(
        &mut self,
        draw_opts: &mut crate::gui::speedy2d::layout::EditorWindowLayoutContentDrawOptions,
        input: &mut crate::gui::speedy2d::layout::UserInput,
    ) -> Option<usize> {
        ...
        if 0.0 < input.clonable.mouse_pos.0
            && input.clonable.mouse_pos.0 < 1.0
            && input.clonable.mouse_pos.1 > y_top_border_tab_row
            && input.clonable.mouse_pos.1 < 1.0
        {
            let num_of_tabs = self.tabs_info.len();

            let tab_info = Self::get_tab_info(
                num_of_tabs,
                draw_opts.my_size_in_pixels.0,
                draw_opts.my_size_in_pixels.1 * tab_row_height,
            );

            let tabs_per_row = tab_info.1;

            let tabs_row = ((input.clonable.mouse_pos.1 - y_top_border_tab_row) * tab_info.0 as f32
                / tab_row_height)
                .floor() as usize;
            ...
            let within_row_mouse_index = (match tabs_row % 3 {
                1 => mouse_x_abs - (draw_opts.my_size_in_pixels.0 - this_row_tabs_width),
                2 => mouse_x_abs.clone(),
                _ => mouse_x_abs - (draw_opts.my_size_in_pixels.0 - this_row_tabs_width) / 2.0,
            } / tab_info.2)
                .floor();

            let index = if within_row_mouse_index >= 0.0 {
                Some(prev_rows_tabs + within_row_mouse_index as usize)
            } else {
                None
            };
