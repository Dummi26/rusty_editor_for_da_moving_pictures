...
use crate::{
    ...
    files,
    gui::speedy2d::layout::{EditorWindowLayoutContentSDrawMode, EditorWindowLayoutContentTrait},
    project::{self, Project, ProjectData, SharedCurves},
    video_render_settings::{FrameRenderInfo, VideoRenderSettings},
};
use speedy2d::{
    color::Color,
    window::{VirtualKeyCode, WindowHandler},
};

use self::{
    content_list::{EditorWindowLayoutContent, EditorWindowLayoutContentEnum},
    editor_window_settings::EditorWindowSettings,
    layout::{
        CustomDrawActions, EditorWindowLayoutContentDrawMode, EditorWindowLayoutContentDrawOptions,
    },
};

pub fn main(args: crate::cli::CustomArgs) -> crate::cli::CustomArgs {
    eprintln!("{}", Clz::starting("Launching speedy2d gui..."),);
    match speedy2d::Window::new_centered("rusty editor for da moving pictures", (1280, 720)) {
        ...
        Err(err) => panic!(
            "\n{}{}\n",
            Clz::error_info("Error creating window with speedy2d: "),
...
pub enum EditorWindowHandlerDrawMode {
    ...
    Transition(
        EditorWindowLayoutContentSDrawMode,
        EditorWindowLayoutContentSDrawMode,
        Instant,
        Duration,
    ),
...
impl EditorWindowHandlerDrawMode {
    ...
    pub fn start_transition(
        &mut self,
        new_mode: EditorWindowLayoutContentSDrawMode,
        duration: Duration,
    ) {
        self.start_transition_be(new_mode, duration, false)
    }
    pub fn start_transition_allow_reverse(
        &mut self,
        new_mode: EditorWindowLayoutContentSDrawMode,
        duration: Duration,
    ) {
        self.start_transition_be(new_mode, duration, true)
    }
    pub fn start_transition_be(
        &mut self,
        new_mode: EditorWindowLayoutContentSDrawMode,
        duration: Duration,
        allow_reverse: bool,
    ...
    pub fn get_draw_mode(&mut self) -> EditorWindowLayoutContentDrawMode {
        match self {
            EditorWindowHandlerDrawMode::Static(v) => {
                EditorWindowLayoutContentDrawMode::Static(v.clone())
            }
            EditorWindowHandlerDrawMode::Transition(old, new, started, duration) => {
                ...
                if prog > 1.0 {
                    ...
                    *self = new_self;
                    ...
                    EditorWindowLayoutContentDrawMode::Transition {
                        modes: [old.clone(), new.clone()],
                        prog,
                    }
                }
            }
...
impl EditorWindowHandler {
    pub fn new(args: &crate::cli::CustomArgs) -> Self {
        fn project_gen_new_empty() -> Project {
            Project::new(ProjectData::default())
        }

        let mut project = match &args.project_path {
            Some(path) => match files::file_handler::read_from_file(path) {
                Ok(Ok(proj)) => {
                    eprintln!("{}", Clz::progress("Loaded project."),);
                    proj
                }
                Ok(Err(err)) => {
                    panic!(
                        "\n{}{}{}\n    {}\n",
                        Clz::error_info("Error parsing project file '"),
                        Clz::error_cause(path.to_string_lossy().as_ref()),
                        Clz::error_info("':"),
                        Clz::error_details(err.to_string().as_str()),
                    );
                ...
                Err(err) => {
                    panic!(
                        "\n{}{}{}\n    {}\n",
                        Clz::error_info("IO-Error loading project from file '"),
                        Clz::error_cause(path.to_string_lossy().as_ref()),
                        Clz::error_info("':"),
                        Clz::error_details(err.to_string().as_str()),
                    );
            ...
            None => {
                eprintln!(
                    "{}",
                    Clz::progress(
                        "No path was provided, so a new (empty) project will be created."
                    ),
                );
                project_gen_new_empty()
            }
        ...
        Self {
            ...
            draw_opts: EditorWindowLayoutContentDrawOptions {
                ...
                assets_manager,
                draw_mode: EditorWindowLayoutContentDrawMode::Static(
            ...
            draw_mode: EditorWindowHandlerDrawMode::Static(
                EditorWindowLayoutContentSDrawMode::Normal,
            ),
            gui_visibility: Some((now, |t| {
                ...
                if elapsed >= 1.0 {
                    ...
                    (0.5 - (elapsed * std::f32::consts::PI).cos() / 2.0, false)
                }
            })),
            ...
            layout: content::layout::half::Half::new(
                [
                    content::layout::half::Half::new(
                        [
                            content::video_preview::VideoPreview::new(project.vid().clone())
                                .as_enum(),
                            content::layout::half::Half::new(
                                [
                                    content::video_tree::VideoTree::new(project.vid().clone())
                                        .as_enum(),
                                    content::video_properties_editor::VideoPropertiesEditor::new(
                                        project.vid().clone(),
                                    )
                                    .as_enum(),
                                ],
                    ...
                    .as_enum(),
                    content::placeholder::Placeholder::new().as_enum(),
                ],
                true,
                0.8,
            )
...
impl WindowHandler for EditorWindowHandler {
    fn on_start(
        ...
        eprintln!(
            ...
            Clz::completed_info("Scale factor:  "),
            Clz::completed_info(info.scale_factor().to_string().as_str()),
            Clz::completed_info("Viewport size: "),
            Clz::completed_info({
                let s = info.viewport_size_pixels();
                format!("{}x{}", s.x, s.y).as_str()
            }),
        );
    }
    ...
    fn on_resize(
        ...
        eprintln!(
            "{}{}",
            Clz::progress("Resized: "),
            Clz::progress(
                format!(
                    "{}x{} ({}MP)-> {}x{} ({}MP)",
                    self.size.0,
                    self.size.1,
                    megapixels(self.size.0, self.size.1),
                    size_pixels.x,
                    size_pixels.y,
                    megapixels(size_pixels.x, size_pixels.y)
                )
                .as_str()
            ),
        );
    ...
    fn on_draw(
        ...
        if let Some(anim_start_time) = self.qvidrunner_state.2 {
            if let EditorWindowLayoutContentEnum::LayoutHalf(half) = &mut self.layout.c {
                let progress =
                    anim_start_time.elapsed().as_secs_f32() / self.qvidrunner_state.3.as_secs_f32();
        ...
        self.draw_opts.visibility =
            self.draw_opts.general_visibility * self.draw_opts.visibility_factors.qvidrunner;
    ...
    fn on_mouse_move(
        ...
        self.mouse_pos = (
            position.x / self.size.0 as f32,
            position.y / self.size.1 as f32,
        );
        self.layout.handle_input(
            &mut self.draw_opts,
            &mut layout::UserInput::new_no_actions(
                layout::UserInputOwned {
                    ...
                    keyboard_modifiers_state: &self.keyboard_modifiers_state,
                    keyboard_down_buttons_scancode: &self.keyboard_down_buttons_scancode,
                    keyboard_down_buttons_virtualkeycode: &self
                        .keyboard_down_buttons_virtualkeycode,
                },
                layout::UserInputClonable {
                    mouse_pos: self.mouse_pos.clone(),
                },
    ...
    fn on_mouse_button_down(
        ...
        match self.mouse_down_buttons.get_mut(&button) {
            Some(v) => {}
            None => {
                self.mouse_down_buttons.insert(
                    button,
                    MouseButtonDownInfo {
    ...
    fn on_key_down(
        ...
        match self.keyboard_down_buttons_scancode.get_mut(&scancode) {
            Some(v) => *v += 1,
            None => {
                self.keyboard_down_buttons_scancode.insert(scancode, 0);
            }
        };
        if let Some(vkc) = virtual_key_code {
            match self.keyboard_down_buttons_virtualkeycode.get_mut(&vkc) {
                Some(v) => *v += 1,
                None => {
                    self.keyboard_down_buttons_virtualkeycode.insert(vkc, 0);
                }
            };
        };
        ...
        self.layout.handle_input(
            ...
            &mut layout::UserInput::new_no_actions(
                layout::UserInputOwned {
                    action: layout::InputAction::Keyboard(layout::KeyboardAction::Pressed(
                        scancode,
                        virtual_key_code,
                    )),
                    project: &mut self.project,
    ...
    fn on_key_up(
        ...
        if let Some(vkc) = virtual_key_code {
            match vkc {
                VirtualKeyCode::Escape => match &self.draw_mode {
                    EditorWindowHandlerDrawMode::Static(mode)
                    | EditorWindowHandlerDrawMode::Transition(_, mode, _, _) => match mode {
                        EditorWindowLayoutContentSDrawMode::Normal => {
                            self.draw_mode.start_transition_allow_reverse(
                                EditorWindowLayoutContentSDrawMode::TypePreview { moving: false },
                                self.settings.switch_modes_duration,
                            )
                        }
                        EditorWindowLayoutContentSDrawMode::TypePreview { moving: _ } => {
                            self.draw_mode.start_transition_allow_reverse(
                                EditorWindowLayoutContentSDrawMode::Normal,
                                self.settings.switch_modes_duration,
                            )
                    ...
                    if self.keyboard_modifiers_state.ctrl() {
                        ...
                        if self.qvidrunner_state.1 == false {
                            ...
                            self.layout.replace(
                                content::layout::half::Half::new(
                                    [
                                        content::special::qvidrunner::QVidRunner::new(
                                            content::special::qvidrunner::QVidRunnerMode::Normal,
                                            self.edited_part,
                                            self.project.clone(),
                                        )
                                        .as_enum(),
                            ...
                            if let EditorWindowLayoutContentEnum::LayoutHalf(half) =
                                &mut self.layout.c
                            {
        ...
        self.layout.handle_input(
            ...
            &mut layout::UserInput::new_no_actions(
                layout::UserInputOwned {
                    action: layout::InputAction::Keyboard(layout::KeyboardAction::Released(
                        scancode,
                        virtual_key_code,
                    )),
                    project: &mut self.project,
    ...
    fn on_keyboard_char(
        ...
        self.layout.handle_input(
            ...
            &mut layout::UserInput::new_no_actions(
                layout::UserInputOwned {
                    action: layout::InputAction::Keyboard(layout::KeyboardAction::Typed(
                        unicode_codepoint,
                    )),
                    project: &mut self.project,
                    mouse_down_buttons: &self.mouse_down_buttons,
